...
# Единая точка входа generate_llm_report() выбирает провайдера по переменной
# LLM_PROVIDER (.env) и просто пересылает готовый prompt в его API:
#   mock             -> без сети, пересказывает ANALYSIS_JSON из промпта (по умолчанию)
#   openai_compatible -> любой сервер с /v1/chat/completions
#   ollama           -> локальный Ollama (/api/chat)
#   cohere           -> Cohere v2 chat API
#   alemplus         -> AlemPlus (тоже OpenAI-совместимый)
# Все реальные провайдеры используют общий _post_json()/_call_openai_chat_completions()
# для HTTP-запросов и единообразной обработки ошибок (timeout, HTTP >=400, битый JSON).
# Модуль не знает ничего про arXiv или relevance score — он только отправляет
# уже готовый текстовый prompt и возвращает текстовый ответ.
...
class Settings(BaseSettings):
    # Провайдер по умолчанию — mock, чтобы демо работало без API-ключей.
...
async def generate_llm_report(prompt: str) -> str:
    # Роутер: по значению LLM_PROVIDER выбираем, какая функция обработает prompt.
    if not prompt.strip():
...
def generate_mock_report(prompt: str) -> str:
    # Не ходит в сеть: достает уже посчитанный ANALYSIS_JSON прямо из текста
    # промпта и просто форматирует его в читаемый отчет. Используется, когда
    # LLM_PROVIDER=mock (по умолчанию) — так демо работает без API-ключей.
    analysis = _extract_analysis_json(prompt)
    query = analysis.get("query", "your topic")
    average = analysis.get("average_relevance", "unknown")
    keywords = analysis.get("top_keywords") or []
    papers = analysis.get("papers") or []

    lines = [
        "[MOCK LLM REPORT]",
        f"Deterministic analysis found {len(papers)} papers for '{query}' "
        f"with an average relevance of {average}/100.",
        "",
    ]

    for index, item in enumerate(papers[:3], start=1):
        paper = item.get("paper", {})
        lines.append(
            f"{index}. {paper.get('title', 'Untitled')} "
            f"(relevance {item.get('relevance_score', '?')}/100) — "
            f"summary excerpt: {paper.get('summary', '')[:160]}..."
        )

    if papers:
        best = papers[0].get("paper", {}).get("title", "the top paper")
        lines += ["", f"Suggested starting point: '{best}' — it has the highest deterministic relevance."]

    if keywords:
        lines += ["", f"Keywords to explore next: {', '.join(keywords[:6])}."]

    lines += [
        "",
        "Remember: this mock report did not calculate relevance; it only restates backend-provided data.",
    ]
    return "\n".join(lines)
...
async def call_ollama(prompt: str) -> str:
    # Ollama использует свой формат ответа (data.message.content), отличный
    # от OpenAI-совместимого (data.choices[0].message.content).
...
async def call_cohere(prompt: str) -> str:
    # У Cohere v2 chat свой формат ответа (message.content — строка или список
    # частей), поэтому парсинг вынесен в отдельную _extract_cohere_text().
...
async def _call_openai_chat_completions(
    *,
    ...
    # Общая реализация для любого провайдера с /v1/chat/completions API
    # (используется и для openai_compatible, и для alemplus).
    if not base_url:
...
async def _post_json(
    *,
    ...
    # Общий HTTP POST helper для всех реальных провайдеров: единообразно
    # ловит сетевые ошибки, HTTP-ошибки и невалидный JSON в ответе.
...
def _extract_analysis_json(prompt: str) -> dict[str, Any]:
    # Промпт в main.py заканчивается "ANALYSIS_JSON:\n{...}" — вырезаем все,
    # что после маркера, и парсим как JSON (raw_decode останавливается сам,
    # даже если после JSON случайно оказался лишний текст).
